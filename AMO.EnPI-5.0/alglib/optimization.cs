    ...
    public class optserv
    {
        ...
        *************************************************************************/
            ...
            double maxsteplen)
        {
            ...
            for(i=0; i<=nmain-1; i++)
            {
                if( (double)(x[i])!=(double)(xprev[i]) && ((havebndl[i] && (double)(x[i])==(double)(bndl[i])) || (havebndu[i] && (double)(x[i])==(double)(bndu[i]))) )
                {
                    result = result+1;
                }
            }
            for(i=0; i<=nslack-1; i++)
            {
                if( (double)(x[nmain+i])!=(double)(xprev[nmain+i]) && (double)(x[nmain+i])==(double)(0.0) )
                {
                    result = result+1;
                }
            }
    ...
    public class minqp
    {
        ...
        *************************************************************************/
        public class minqpstate
        {
            ...
            public double[] gc;
            public double[] dc;
            public double[] xcand0;
            public double[] xcand1;
            public double[] xn;
            public double[] pg;
        ...
        *************************************************************************/
            ...
            minqpstate state)
        {
            ...
            state.xcand0 = new double[n];
            state.xcand1 = new double[n];
            state.xc = new double[n];
            state.xn = new double[n];
            state.gc = new double[n];
            state.pg = new double[n];
            state.dc = new double[n];
            state.activeset = new bool[n];
            for(i=0; i<=n-1; i++)
            {
                state.b[i] = 0.0;
        ...
        *************************************************************************/
        public static void minqpoptimize(minqpstate state)
        {
            ...
            double v1 = 0;
            double vn0 = 0;
            bool constraintsactivated = new bool();
            bool constraintsdeactivated = new bool();
            bool noisystep = new bool();
            double v = 0;
            ...
            if( state.algokind==1 && state.akind==0 )
            {
                ...
                if( nbc==0 && nlc==0 )
                {
                    ...
                    for(i=0; i<=n-1; i++)
                    {
                        for(j=i; j<=n-1; j++)
                        {
                            state.bufa[i,j] = state.densea[i,j];
                        }
                    }
                    for(i_=0; i_<=n-1;i_++)
                    {
                    ...
                    if( !trfac.spdmatrixcholeskyrec(ref state.bufa, 0, n, true, ref state.tmp0) )
                    {
                        state.repterminationtype = -5;
                        return;
                    }
                    fbls.fblscholeskysolve(state.bufa, 1.0, n, true, ref state.bufb, ref state.tmp0);
                    for(i_=0; i_<=n-1;i_++)
                    {
                ...
                alglib.ap.assert(nlc==0, "MinQPOptimize: internal error");
                ...
                while( true )
                {
                    ...
                    for(i=0; i<=n-1; i++)
                    {
                        state.activeset[i] = (state.havebndl[i] && (double)(state.xc[i])==(double)(state.workbndl[i])) || (state.havebndu[i] && (double)(state.xc[i])==(double)(state.workbndu[i]));
                    }
                    state.repncholesky = state.repncholesky+1;
                    if( !minqppreparenewtonstep(state, state.xc, state.activeset, ref state.xn, n, ref state.bufa, ref state.tmp0, ref state.tmp1) )
                    {
                        state.repterminationtype = -5;
                        return;
                    }
                    minqpstepto(state, state.xc, ref state.xn, state.workbndl, state.havebndl, state.workbndu, state.havebndu, n, ref state.tmp0, ref state.tmp1, ref constraintsactivated, ref constraintsdeactivated, ref noisystep);
                    for(i_=0; i_<=n-1;i_++)
                    {
                    ...
                    //
                    minqpgrad(state, ref state.xc, ref state.gc);
                    for(i_=0; i_<=n-1;i_++)
                    {
                        state.pg[i_] = state.gc[i_];
                    }
                    minqpscalegrad(state, ref state.pg);
                    optserv.projectgradientintobc(state.xc, ref state.pg, state.workbndl, state.havebndl, state.workbndu, state.havebndu, n, 0);
                    minqptargetfunction(state, state.xc, ref v, ref vn0);
                    v0 = minqpxtax2(state, state.pg);
                    ...
                    for(i=0; i<=n-1; i++)
                    {
                        state.activeset[i] = false;
                        state.activeset[i] = state.activeset[i] || ((state.havebndl[i] && (double)(state.xc[i])==(double)(state.workbndl[i])) && (double)(state.gc[i])>(double)(0));
                        state.activeset[i] = state.activeset[i] || ((state.havebndu[i] && (double)(state.xc[i])==(double)(state.workbndu[i])) && (double)(state.gc[i])<(double)(0));
                    }
                    if( !minqppreparenewtonstep(state, state.xc, state.activeset, ref state.xn, n, ref state.bufa, ref state.tmp0, ref state.tmp1) )
                    {
                        state.repterminationtype = -5;
        ...
        *************************************************************************/
            ...
            double s)
        {
            ...
            if( isupper )
            {
                ...
                state.densea[n-1,n-1] = a[n-1,n-1]+s;
            }
            else
            {
                state.densea[0,0] = a[0,0]+s;
                for(k=1; k<=n-1; k++)
                {
                    for(i_=0; i_<=k-1;i_++)
                    {
        ...
        *************************************************************************/
            ...
            double[] s)
        {
            ...
            alglib.ap.assert(state.akind==0, "MinQPRewriteDiagonal: internal error (AKind<>0)");
            n = state.n;
            for(k=0; k<=n-1; k++)
            {
                state.densea[k,k] = s[k];
            }
        }
        ...
        *************************************************************************/
            ...
            ref double[] gc)
        {
            ...
            n = state.n;
            alglib.ap.assert(state.akind==-1 || state.akind==0, "MinQPGrad: internal error");
            ...
            if( state.akind==0 )
            {
                ...
                gc[0] = gc[0]+state.densea[0,0]*xc[0];
                for(i=1; i<=n-1; i++)
                {
                    v = 0.0;
                    for(i_=0; i_<=i-1;i_++)
                    {
                        v += state.densea[i,i_]*xc[i_];
                    }
                    gc[i] = gc[i]+v+state.densea[i,i]*xc[i];
                    v = xc[i];
                    for(i_=0; i_<=i-1;i_++)
                    {
                        gc[i_] = gc[i_] + v*state.densea[i,i_];
                    }
                }
                return;
            }
        ...
        It MUST be called only for problems with SPD matrices.
          ...
          -- ALGLIB --
             Copyright 11.01.2011 by Bochkanov Sergey
        *************************************************************************/
        private static void minqpscalegrad(minqpstate state,
            ref double[] g)
        {
            int n = 0;
            int i = 0;
            ...
            n = state.n;
            alglib.ap.assert(state.akind==0, "MinQPGrad: internal error");
        ...
        This  function  calculates 0.5*x'*A*x for given X.
          ...
          -- ALGLIB --
             Copyright 11.01.2011 by Bochkanov Sergey
        *************************************************************************/
        private static double minqpxtax2(minqpstate state,
            double[] x)
        {
            double result = 0;
            int n = 0;
            ...
            n = state.n;
            alglib.ap.assert(state.akind==-1 || state.akind==0, "MinQPXTAX: internal error");
            result = 0;
            ...
            if( state.akind==0 )
            {
                ...
                for(i=0; i<=n-1; i++)
                {
                    ...
                    result = result+0.5*state.densea[i,i]*math.sqr(x[i]);
                }
                return result;
            }
            return result;
        ...
        OUTPUT PARAMETERS:
          ...
          -- ALGLIB --
             Copyright 11.01.2011 by Bochkanov Sergey
        *************************************************************************/
        private static void minqptargetfunction(minqpstate state,
            double[] x,
            ref double f,
            ref double noise)
        {
            ...
            if( state.akind==-1 )
            {
                ...
                for(i=0; i<=n-1; i++)
                {
                    v = state.b[i]*x[i];
                    f = f+v;
                    noise = Math.Max(noise, Math.Abs(v));
                }
                
                //
                // End
                //
                noise = math.machineepsilon*n*noise;
                return;
            }
            ...
            if( state.akind==0 )
            {
                ...
                for(i=0; i<=n-1; i++)
                {
                    for(j=0; j<=i-1; j++)
                    {
                        v = state.densea[i,j]*x[i]*x[j];
                        f = f+v;
                        noise = Math.Max(noise, Math.Abs(v));
                    }
                    v = 0.5*state.densea[i,i]*math.sqr(x[i]);
                    f = f+v;
                    noise = Math.Max(noise, Math.Abs(v));
                }
        ...
        *************************************************************************/
        private static bool minqppreparenewtonstep(minqpstate state,
            double[] xc,
            bool[] activeset,
            ref double[] xn,
            int n,
            ...
            ref double[] bufc)
        {
            ...
            alglib.ap.assert(state.akind==0, "MinQPNewtonStep: unexpected model type");
            ...
            //
            apserv.rmatrixsetlengthatleast(ref bufa, n, n);
            apserv.rvectorsetlengthatleast(ref bufb, n);
            for(i=0; i<=n-1; i++)
            {
                if( activeset[i] )
                {
            ...
            for(i=0; i<=n-1; i++)
            {
                if( !activeset[i] )
                {
                    bufb[nfree] = -state.b[i];
                    k = 0;
                    for(j=0; j<=n-1; j++)
                    {
                        if( !activeset[j] )
                        {
                            bufa[nfree,k] = state.densea[i,j];
                            k = k+1;
                        }
                        else
                        {
                            bufb[nfree] = bufb[nfree]-state.densea[i,j]*xn[j];
                        }
                    }
                    nfree = nfree+1;
                }
            ...
            if( nfree>0 )
            {
                if( !trfac.spdmatrixcholeskyrec(ref bufa, 0, nfree, true, ref bufc) )
                {
                    result = false;
                    return result;
                }
                fbls.fblscholeskysolve(bufa, 1.0, nfree, true, ref bufb, ref bufc);
                k = 0;
                for(i=0; i<=n-1; i++)
                {
                    if( !activeset[i] )
        ...
        * XN, candidate point, which minimizes function value in some sense (either
        ...
        NOTE 1: it is important that XN is constrained only by constraints,  which
          ...
          -- ALGLIB --
             Copyright 29.02.2012 by Bochkanov Sergey
        *************************************************************************/
        private static void minqpstepto(minqpstate state,
            double[] xc,
            ref double[] xn,
            double[] bndl,
            bool[] havebndl,
            ...
            ref double[] tmp0,
            ref double[] tmp1,
            ref bool constraintsactivated,
            ref bool constraintsdeactivated,
            ref bool noisystep)
        {
            int variabletofreeze = 0;
            double valuetofreeze = 0;
            double maxsteplen = 0;
            double armijostep = 0;
            int armijocnt = 0;
            double v = 0;
            ...
            constraintsactivated = new bool();
            constraintsdeactivated = new bool();
            noisystep = new bool();

            apserv.rvectorsetlengthatleast(ref tmp0, n);
            apserv.rvectorsetlengthatleast(ref tmp1, n);
            
            //
            // Compare function values at XC and XN
            //
            minqptargetfunction(state, xc, ref v0, ref v);
            minqptargetfunction(state, xn, ref v1, ref v);
            if( (double)(v1)>=(double)(v0) )
            {
                constraintsactivated = false;
                constraintsdeactivated = false;
                noisystep = true;
                for(i_=0; i_<=n-1;i_++)
                {
                    xn[i_] = xc[i_];
            ...
            for(i_=0; i_<=n-1;i_++)
            {
                tmp0[i_] = tmp0[i_] - xc[i_];
            }
            optserv.calculatestepbound(xc, tmp0, 1.0, bndl, havebndl, bndu, havebndu, n, 0, ref variabletofreeze, ref valuetofreeze, ref maxsteplen);
            if( variabletofreeze>=0 && (double)(maxsteplen)<=(double)(1) )
            {
                ...
                for(i_=0; i_<=n-1;i_++)
                {
                    tmp0[i_] = maxsteplen*tmp0[i_];
                }
                for(i_=0; i_<=n-1;i_++)
                {
                    tmp0[i_] = tmp0[i_] + xc[i_];
                }
                optserv.postprocessboundedstep(ref tmp0, xc, bndl, havebndl, bndu, havebndu, n, 0, variabletofreeze, valuetofreeze, maxsteplen, maxsteplen);
            }
            else
            {
            ...
            if( variabletofreeze>=0 && (double)(maxsteplen)<=(double)(1) )
            {
                ...
                //
                minqptargetfunction(state, tmp0, ref v0, ref v);
                armijostep = 1.0;
                armijocnt = 5;
                for(i_=0; i_<=n-1;i_++)
                {
                ...
                while( armijocnt>0 )
                {
                    ...
                    for(i_=0; i_<=n-1;i_++)
                    {
                        tmp1[i_] = tmp1[i_] - tmp0[i_];
                    }
                    for(i_=0; i_<=n-1;i_++)
                    {
                        tmp1[i_] = armijostep*tmp1[i_];
                    }
                    for(i_=0; i_<=n-1;i_++)
                    {
                        tmp1[i_] = tmp1[i_] + tmp0[i_];
                    ...
                    for(i=0; i<=n-1; i++)
                    {
                        if( havebndl[i] && (double)(tmp1[i])<(double)(bndl[i]) )
                        {
                            tmp1[i] = bndl[i];
                        }
                        if( havebndu[i] && (double)(tmp1[i])>(double)(bndu[i]) )
                        {
                            tmp1[i] = bndu[i];
                        }
                    }
                    ...
                    // Evaluate function
                    //
                    minqptargetfunction(state, tmp1, ref v1, ref v);
                    if( (double)(v1)>=(double)(v0) )
                    {
                        for(i_=0; i_<=n-1;i_++)
                        {
            ...
            for(i=0; i<=n-1; i++)
            {
                b0 = (havebndl[i] && (double)(bndl[i])==(double)(xc[i])) || (havebndu[i] && (double)(bndu[i])==(double)(xc[i]));
                b1 = (havebndl[i] && (double)(bndl[i])==(double)(xn[i])) || (havebndu[i] && (double)(bndu[i])==(double)(xn[i]));
                constraintsactivated = constraintsactivated || (b1 && !b0);
                constraintsdeactivated = constraintsdeactivated || (b0 && !b1);
            }
            minqptargetfunction(state, xc, ref v0, ref vn0);
            minqptargetfunction(state, xn, ref v1, ref vn1);
            noisystep = (double)(v0-v1)<(double)(Math.Max(vn0, vn1));
        }
